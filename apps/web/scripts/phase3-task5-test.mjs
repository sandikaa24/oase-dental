...
async function runSuite() {
  ...
  // --- SECTION 3: MASTER PRODUK (PRODUCTS) ---
  console.log('\n--- MD-3: Master Produk Jual (CRUD, SKU Unique, & Soft/Hard Delete) ---');
  const prodSku = `SKU-PRD-${rnd}`;
  const prodName = `Pasta Gigi Herbal ${rnd}`;

  // 1. Create Product
  res = await fetch(`${BASE_URL}/api/v1/products`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Cookie: ownerAuth.cookies },
    body: JSON.stringify({
      name: prodName,
  ...
  body = await res.json();
  assert(res.status === 201 && body.success, 'MD-3.1: OWNER berhasil membuat Produk Jual baru (201 Created)');
  ...
  body = await res.json();
  assert(res.status === 409 && body.code === 'DUPLICATE', 'MD-3.2: Pembuatan produk dengan SKU duplikat ditolak (409 DUPLICATE)');

  // 3. Update Product & Verify Match on GET
  res = await fetch(`${BASE_URL}/api/v1/products/${prodId}`, {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json', Cookie: ownerAuth.cookies },
    body: JSON.stringify({
      sellPrice: 55000,
  ...
  assert(res.status === 200, 'MD-3.3: PATCH produk berhasil disimpan (200 OK)');

  res = await fetch(`${BASE_URL}/api/v1/products/${prodId}`, {
    headers: { Cookie: ownerAuth.cookies },
  });
  body = await res.json();
  const fetchedProdPrice = typeof body.data?.sellPrice === 'string' ? Math.round(Number(body.data.sellPrice)) : body.data?.sellPrice;
  assert(
    res.status === 200 && fetchedProdPrice === 55000 && body.data?.minStock === 15,
    'MD-3.4: Data hasil GET ulang produk persis cocok dengan field hasil PATCH'
  );

  // 4. Hard Delete pada Unused Product
  res = await fetch(`${BASE_URL}/api/v1/products/${prodId}`, {
    method: 'DELETE',
    headers: { Cookie: ownerAuth.cookies },
  });
  body = await res.json();
  assert(res.status === 200 && body.data?.mode === 'hard', 'MD-3.5: DELETE produk yang belum pernah ditransaksikan menghasilkan mode: "hard"');

  // 5. Soft Delete pada Used Product (Stock In)
  const usedProdSku = `SKU-USED-PRD-${rnd}`;
  res = await fetch(`${BASE_URL}/api/v1/products`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Cookie: ownerAuth.cookies },
    body: JSON.stringify({
      name: `Produk Dipakai ${rnd}`,
      sku: usedProdSku,
      sellPrice: 25000,
      unit: 'pcs',
      minStock: 5,
  ...
  await fetch(`${BASE_URL}/api/v1/inventory/stock-in`, {
    ...
    body: JSON.stringify({
      itemType: 'PRODUCT',
      items: [{ itemId: usedProdId, quantity: 10, unitCost: 15000 }],
      note: 'Stock in produk pengujian',
    }),
  });

  // Hapus used product -> mode: soft
  res = await fetch(`${BASE_URL}/api/v1/products/${usedProdId}`, {
    method: 'DELETE',
    headers: { Cookie: ownerAuth.cookies },
  });
  body = await res.json();
  assert(res.status === 200 && body.data?.mode === 'soft', 'MD-3.6: DELETE produk yang memiliki riwayat inventaris menghasilkan mode: "soft" (Soft Delete)');

  // 6. CASHIER Guard on Products
  res = await fetch(`${BASE_URL}/api/v1/products`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Cookie: cashierAuth.cookies },
    body: JSON.stringify({ name: `Kasir Prod ${rnd}`, sku: `KSR-${rnd}`, sellPrice: 1000, unit: 'pcs' }),
  });
  assert(res.status === 403, 'MD-3.7: CASHIER ditolak saat membuat produk (403 FORBIDDEN)');
